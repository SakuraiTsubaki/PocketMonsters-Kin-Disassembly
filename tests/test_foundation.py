...
class FoundationTests(unittest.TestCase):
    ...
    def test_project_identity(self):
        ...
        self.assertEqual(project["id"], "kin")
        self.assertEqual(project["repository"], "SakuraiTsubaki/PocketMonsters-Kin-Disassembly")
        self.assertEqual(project["architecture"], "gb-sm83")
    ...
    def test_rom_extensions_and_archived_roms_are_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "rebuilt.gbc").write_bytes(b"rom")
